...
Page({
    ...
    onShow: function(options) {
        let self = this
        //每次进入 清空leftcate
		let cacheCategory = wx.getStorageSync("cacheCategory")
		if (Object.keys(cacheCategory).length === 0 && cacheCategory.constructor === Object) {
			self.setData({
				rightCategory: [],
				curIndex:0
			})
			...
			self.setData({
				curIndex: cacheCategory['curIndex'],
				rightCategory:cacheCategory['rightCategory']
			})
		}
        ...
        self.filterProducts();
        self.setData({
            brands: self.filterBrands()
        });
        self.setData({
            items: self.data.brands
        });
    },

    loadCategoies: function() {
        let self = this;
        let res = wx.getStorageSync("categories")
        ...
        self.setData({
            res: res,
            leftCategory: self.data.leftCategory
        });
        // })
