...
Page({
    ...
    listProducts: function() {
        ...
        COM.load('NetUtil').netUtil(url, "GET", "", (callbackdata) => {
            ...
            products.slice(0, products.length).forEach(function(p) {
                p.thumb = COM.load('Util').imageThumb(p.barcode);
    ...
    searchByBarcode: function(barcode) {
        ...
        COM.load('NetUtil').netUtil(COM.load('CON').BAR_CODE_URL + barcode, "GET", "", function(res) {
            ...
            if (item) {
				...
				item.thumb = COM.load('Util').imageThumb(item.barcode);
                item.selected = myProductsIds.includes(item.id.toString());
    ...
    showDetail: function(e) {
        ...
        let products = self.data.goodsLineList
        self.setData({
            productDetailModal: true,
            productDetail: products[key]
        })
    ...
    showDetailSearch: function(e) {
        let self = this
        let key = e.currentTarget.dataset.key;
        let products = self.data.searchResult
    ...
    hideProductDetailModal: function(e) {
        this.setData({
            productDetailModal: false,
        })
    }
